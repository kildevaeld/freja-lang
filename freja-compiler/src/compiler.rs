...
impl Compiler {
    ...
    fn make_constant(&mut self, value: Constant) -> usize {
    ...
    fn parse_var(&mut self, name: &str) -> CompileResult<usize> {
        ...
        if self.state().scope_depth > 0 {
            ...
            Ok(self.make_constant(Constant::String(name.to_string())))
        }
    }
    ...
    fn function2(
        ...
        let constant = self.make_constant(Constant::Function(Rc::new(function))) as u8;
    ...
    fn variable(&mut self, name: &str) {
        let (a, get, set) = if let Some(a) = self.resolve_local(name) {
            ...
            let a = self.make_constant(Constant::String(name.to_owned()));
...
impl StmtVisitor<CompileResult<()>> for Compiler {
    ...
    fn visit_class_stmt(&mut self, e: &ClassStmt) -> CompileResult<()> {
        let global = self.make_constant(Constant::String(e.name.to_string()));
        ...
        for m in &e.members {
            match m.as_ref() {
                Stmt::Func(stmt) => {
                    ...
                    let name = self.make_constant(Constant::String(stmt.name.to_string()));
...
impl ExprVisitor<CompileResult<()>> for Compiler {
    ...
    fn visit_call_expr(&mut self, e: &CallExpr) -> CompileResult<()> {
        ...
        match e.member.as_ref() {
            Expr::Member(mem) => {
                ...
                let name = self.make_constant(Constant::String(name.to_string()));
    ...
    fn visit_literal_expr(&mut self, e: &LiteralExpr) -> CompileResult<()> {
        ...
        let val = match &e.value {
            Literal::Number(Number::Double(d)) => Constant::Double(*d),
            Literal::Number(Number::Integer(i)) => Constant::Integer(*i),
    ...
    fn visit_identifier_expr(&mut self, e: &IdentifierExpr) -> CompileResult<()> {
        if self.state().member_depth > 0 {
            let global = self.make_constant(Constant::String(e.value.to_string()));
            if self.state().assign {
    ...
    fn visit_postfix_expr(&mut self, e: &PostfixExpr) -> CompileResult<()> {
        ...
        self.emit_constant(Constant::Integer(1));
