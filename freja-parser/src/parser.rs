    ...
    let lit = match inner.as_rule() {
        ...
        Rule::numeric_literal => {
            ...
            let n = match p.as_rule() {
                Rule::float => Number::Double(p.as_str().parse::<f64>().expect("double")),
                Rule::integer => Number::Integer(p.as_str().parse::<i64>().expect("integer")),
...
fn parse_bin2<'a>(mut pairs: Vec<Pair<'a, Rule>>, location: Location) -> Expr {
    ...
    let left = Box::new(parse_expression(iter.next().unwrap()));
    ...
    Expr::Binary(BinaryExpr {
        ...
        left,
        operator,
        right: Box::new(parse_bin(iter.collect(), location)),
    })
}
...
fn parse_bin<'a>(mut pairs: Vec<Pair<'a, Rule>>, location: Location) -> Expr {
    ...
    let right = Box::new(parse_expression(pairs.pop().unwrap()));
    ...
    Expr::Binary(BinaryExpr {
        ...
        right,
        operator,
        left: Box::new(parse_bin(pairs, location)),
    })
}
...
fn parse_binray_expr<'a>(pair: Pair<'a, Rule>) -> Expr {
    ...
    return parse_bin(children, location);
